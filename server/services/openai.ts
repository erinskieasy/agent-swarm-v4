...
function getDateContext(): string {
    ...
    return `\n\nCurrent date: ${formatted}. You have access to a web_search tool — use it when you need current, real-world data to produce accurate results.`;
}
...
/**
 * Process any tool calls in the response by executing web searches
...
/**
 * Standard chat completion with web search tool — returns the assistant's text response.
 * The LLM can autonomously decide to search the web when it needs real-world data.
 */
export async function chatCompletion(
    ...
    const messages: ChatCompletionMessageParam[] = [
        { role: 'system', content: systemPrompt + getDateContext() },
    ...
    if (response.choices[0]?.message?.tool_calls?.length) {
        return handleToolCalls(messages, response, model, 0.7);
    }
...
/**
 * JSON chat completion with web search tool — parses the response as JSON of type T.
 * The LLM can search the web before producing its structured JSON output.
 */
export async function chatCompletionJSON<T>(
    ...
    if (response.choices[0]?.message?.tool_calls?.length) {
        const textResult = await handleToolCalls(messages, response, model, 0.7);
